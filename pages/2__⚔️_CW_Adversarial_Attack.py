...
# App title
st.title("CW Adversarial Image Attack Tool")
...
st.info("This is a targeted version of the CW attack see (https://arxiv.org/pdf/1608.04644).")
...
cw.set_mode_targeted_by_label(True)
    ...
    if st.button("Perform Attack"):
        with TemporaryDirectory() as temp_dir:
            ...
            with st.spinner("Generating adversarial image..."):
                input_img_tensor = pipeline.image_processor.process(temp_file_path)
                output_img_tensor = cw(input_img_tensor, torch.tensor([target_class]))
